...
int do_select(string arg)
{
	...
	if ( !wizardp(this_player()) ) 
        {
        command ("say 你无权召开比武大会!\n");
        return 0;
	}
	command("chat 好，现在正式开始挑选"+arg+"候选人!\n");
	set("class",arg);
	for ( i=0;i<sizeof(ob); i++ ) 
	if(!wizardp(ob[i])&&(string)ob[i]->query("family/family_name") == arg) 
	{
	this_object()->set("candidate",ob[i]);
	break;
	}
	for ( i=sizeof(ob); i>0; i-- ) {
	if(!wizardp(ob[i-1])&&(string)ob[i-1]->query("family/family_name") == arg) 
		ob[i-1]->move("/d/huashan/sheshen");
		if(!wizardp(ob[i-1])&&(string)ob[i-1]->query("family/family_name") == arg)
		if(ob[i-1]->query("combat_exp")>this_object()->query("candidate")->query("combat_exp"))
		set("candidate",ob[i-1]);
	}
	command("chat 现在, "+arg+"第一候选人是"+this_object()->query("candidate")->query("name")+"!\n");
	command("chat 大家可以自由挑战。\n任何人连胜三场或者无人再向其挑战即可获得"+arg+"第一的称号。\n");
	command("chat 任何战败或中途弃权者均只能争夺下一称号.\n");
	command("say 你可以用setname somebody 指定某人为当前胜者。\n"+
...
{     
	...
	if (ob1->query_temp("lose")*3>query("win_times"))    return notify_fail("你已经输过一次了,只能向下一候选人挑战。\n");
...
void check(object ob1, object ob2)
{
	...
	command("chat 当前候选人"+query("candidate")->query("name")+"已获胜"+(string)(query("win_times")%3)+"次.\n");
...
int observe(object ob1,object ob2)
{
	...
	if ( !present(ob1, environment()) ) 
	{
		...
		add("win_times",1);
		if(this_object()->query("win_times")==9)
                {
		command("chat " + ob2->query("name") 
			+ "获得本次比武大会"+query("class")+"第三的称号！\n");
		add_title(ob2,""+query("class")+"第三");
		...
		if(query("win_times")==6)
                {
		command("chat " + ob2->query("name") 
			+ "获得本次比武大会"+query("class")+"第二的称号！\n");
		add_title(ob2,query("class")+"第二");
		add_title(ob2,query("class")+"第二");
		command("chat 大家可以自由挑战。任何人连胜三场或者无人再向其挑战即可获得"+query("class")+"第三的称号。\n");
                set("candidate",ob1);
		return 1;
		}		
		if(query("win_times")==3)
                {
		command("chat " + ob2->query("name") 
			+ "获得本次比武大会"+query("class")+"第一的称号！\n");
		add_title(ob2,query("class")+"第一");
		command("chat 现在, "+query("class")+"第二的候选人是"+ob1->query("name")+"！\n");
		command("chat 大家可以自由挑战。任何人连胜三场或者无人再向其挑战即可获得"+query("class")+"第二的称号。\n");
                set("candidate",ob1);
		}
		return 1;
	}
	...
	if ( !present(ob2, environment()) ) 
	{
		...
		if(query("win_times")<3)
		{	
		command("chat " + ob1->query("name")+ "成为"+query("class")+"第一之候选人！\n");
		set("win_times",0);
		}
		else 
		if(query("win_times")<6)
		{
		command("chat " + ob1->query("name")+ "成为"+query("class")+"第二之候选人！\n");
		set("win_times",3);
		}
		else 
		{
		command("chat " + ob1->query("name") + "成为"+query("class")+"第三之候选人！\n");
		set("win_times",6);
		}
		set("candidate",ob1);
		ob2->add_temp("lose",1);
		...
		if(this_object()->query("win_times")==6)
                {
		command("chat " + ob2->query("name") 
			+ "获得本次比武大会"+query("class")+"第二的称号！\n");
		add_title(ob2,query("class")+"第二");
		command("chat 现在, "+query("class")+"第三的候选人是"+ob1->query("name")+"！\n");
		command("chat 大家可以自由挑战。任何人连胜三场或者无人再向其挑战即可获得"+query("class")+"第三的称号。\n");
                set("candidate",ob1);
		return 1;
		}		
		if(this_object()->query("win_times")==3)
                {
		command("chat " + ob2->query("name") 
			+ "获得本次比武大会"+query("class")+"第一的称号！\n");
		add_title(ob2,query("class")+"第一");
...
string ask_candidate()
{
	...
	if(!wizardp(ob[i])) 
	{
	if(!(strsrch((string)ob[i]->query("title"), (string)query("class")) >= 0))
	{
	obj=ob[i];
	break;
	}
	...
	for ( i=sizeof(ob); i>0; i-- ) {
		if(!wizardp(ob[i-1]))
		if(!(strsrch((string)ob[i-1]->query("title"), (string)query("class")) >= 0))
		if((int)ob[i-1]->query("combat_exp")>(int)query("candidate")->query("combat_exp"))
		obj=ob[i-1];
	}
...
{
if( (strsrch(arg, query("class")+"第一") >= 0)||(strsrch(arg, query("class")+"第二") >= 0)||(strsrch(arg, query("class")+"第三") >= 0))
		{
		arg = replace_string(arg, query("class")+"第一","");
		arg = replace_string(arg, query("class")+"第二","");
		arg = replace_string(arg, query("class")+"第三","");
        }
return arg;
}
...
int do_setname(string arg)
{
	...
	if(this_object()->query("win_times")>=6)
		...
		command("chat " + dest->query("name") 
			+ "获得本次比武大会"+query("class")+"第三的称号!\n");
		add_title(dest,query("class")+"第三");
		command("chat 比武大会结束!\n");
		this_object()->delete("candidate");
		this_object()->delete("win_times");
		}		
	else if(this_object()->query("win_times")>=3)
                {
		command("chat " + dest->query("name") 
			+ "获得本次比武大会"+query("class")+"第二的称号!\n");
		add_title(dest,query("class")+"第二");
		set("win_times",6);
		command("chat 现在大家可以自由挑战。争夺"+query("class")+"第三。\n");
                losetime=2;
		}		
		...
		command("chat " + dest->query("name")
			+ "获得本次比武大会"+query("class")+"第一的称号!\n");
		add_title(dest,query("class")+"第一");
		set("win_times",3);
		command("chat 现在大家可以自由挑战。争夺"+query("class")+"第二。\n");
                losetime=1;
		}		
	for(i=0; i<sizeof(all); i++)
