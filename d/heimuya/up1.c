...
void create()
{
	...
	set("long", "      你终于走出了那片阴深的森林, 不由长长地舒了口气. 可是, 一座\n"
      ...
      "看不清晰.从悬崖上垂下一条绳索(rope). 看上去象是悬崖上下互通消息的工具.\n "
      "绳索的上面似乎隐约有一个掉蓝(basket),悬崖的墙上似乎有一个匣子(xiazi),\n"
      "你似乎走入一个雾宅,真不知如何是好!\n" 
        );
    ...
    set("item_desc", ([
        "basket": "那是一个掉蓝,看样子有些古怪,似乎象个机关开动脑筋吗,实在不行就先来点脑黄金,嘻嘻!\n",
    ...
    if(!query("exits/enter") ) {
        ...
        if( room = find_object(__DIR__"basket") ) {
            if((int)room->query("insert_trigger")==0 ) {
                ...
                message("vision", "只见一个掉蓝(basket)从天而降，你不禁一怔！\n"
                    , this_object() );
                set("item_desc", ([
        "basket" : "这是一个掉蓝,是传说中的交通工具.\n",
            ]));
...
void on_board()
{
    ...
    message("vision", "只见掉蓝缓缓向上，四周无限美好，\n"
             "你似乎置身于一处人间仙境无异.\n"   ,
        this_object() );
